...
class GtkAccountUiArchitectureTest extends TestCase
{
    public function test_core_gtk_account_pages_use_the_shared_simansa_visual_language(): void
    {
        ...
        $this->assertStringContainsString('gtk-account-dashboard__quick-link', $dashboard);
        $this->assertStringContainsString('Akses Cepat', $dashboard);
        $this->assertStringContainsString('Agenda Mengajar Hari Ini', $dashboard);
        $this->assertLessThan(
            strpos($dashboard, 'gtk-account-dashboard__quick-access'),
            strpos($dashboard, 'gtk-account-dashboard__profile')
        );
