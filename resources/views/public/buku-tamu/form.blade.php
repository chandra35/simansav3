    ...
    <section class="card guest-card">
        ...
        <div class="card-body p-4 p-md-5">
            ...
            <form method="POST" action="{{ route('public.buku-tamu.store', $token) }}" novalidate>
                ...
                <button class="btn btn-primary btn-lg w-100" type="submit"><span class="submit-label">Simpan Data Kunjungan</span><span class="spinner-border spinner-border-sm d-none" aria-hidden="true"></span></button>
            </form>
        </div>
    </section>
</main>
...
function loadNext(targetId, sourceCode, selected = '') {
    ...
    fetch(config.url(sourceCode), { headers: { Accept: 'application/json' } }).then(response => response.json()).then(items => { target.innerHTML = `<option value="">${config.label}</option>`; items.forEach(item => { const option = new Option(item.name, item.code, false, item.code === selected); target.add(option); }); target.disabled = false; if (config.next && selected) loadNext(config.next, selected, oldValues[config.next]); }).catch(() => { target.innerHTML = `<option value="">Gagal memuat data</option>`; });
}
