...
(() => {
    ...
    function render() {
        ...
        if (displayed) {
            ...
            classes.innerHTML = data.length ? data.map(item => `<article class="piket-class"><i class="fas ${icon(item.mapel)}"></i><div class="piket-kelas">${escape(item.kelas)}</div><div class="piket-mapel">${escape(item.mapel)}</div><div class="piket-guru">${item.foto_guru ? `<img class="piket-guru-photo" src="${escape(item.foto_guru)}" alt="Foto ${escape(item.guru)}" onerror="this.remove()">` : '<i class="fas fa-user-tie"></i>'}<span>${escape(item.guru)}</span></div></article>`).join('') : `<div class="piket-empty">${breakTime ? `${escape(displayed.label)} merupakan kegiatan khusus tanpa jadwal kelas.` : 'Jadwal masih kosong pada jam ini.'}</div>`;
        } else {
