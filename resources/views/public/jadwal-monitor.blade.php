...
(() => {
    ...
    function render() {
        ...
        if (displayed) {
            ...
            classes.innerHTML = data.length ? data.map(item => `<article class="piket-class"><i class="fas ${icon(item.mapel)}"></i><div class="piket-kelas">${escape(item.kelas)}</div><div class="piket-mapel">${escape(item.mapel)}</div><div class="piket-guru"><i class="fas fa-user-tie"></i> ${escape(item.guru)}</div></article>`).join('') : `<div class="piket-empty">${breakTime ? `${escape(displayed.label)} merupakan kegiatan khusus tanpa jadwal kelas.` : 'Jadwal masih kosong pada jam ini.'}</div>`;
        } else {
