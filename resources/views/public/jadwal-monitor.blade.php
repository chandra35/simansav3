...
<head>
    ...
    <style>
        ...
        .piket-footer{display:flex;align-items:center;justify-content:center;gap:9px;margin-top:13px;padding:10px 14px;border:1px solid #dbe5f2;border-radius:12px;background:rgba(255,255,255,.72);color:#64748b;font-size:11px;font-weight:700;letter-spacing:.035em;text-align:center}.piket-footer strong{color:#1e3a8a;font-weight:900}.piket-footer i{color:#0f766e;font-size:10px}.piket-footer__divider{width:4px;height:4px;border-radius:99px;background:#93c5fd}
        @media(max-width:600px){.piket-monitor{padding:10px}.piket-top{align-items:flex-start}.piket-brand img{display:none}.piket-brand small{font-size:8px}.piket-brand strong{font-size:16px}.piket-clock strong{font-size:21px}.piket-fullscreen{display:none}.piket-now{width:42px;padding:0;justify-content:center}.piket-now span{display:none}.piket-classes{grid-template-columns:1fr}.piket-focus{margin:10px 0}.piket-timeline{margin-top:10px}.piket-footer{flex-wrap:wrap;gap:6px;font-size:10px}.piket-footer__divider{display:none}}
    </style>
</head>
<body>
...
(() => {
    ...
    function render() {
        ...
        if (displayed) {
            ...
            const classKey = `${displayed.tipe}:${displayed.jam_ke}:${data.map(item => `${item.kelas}|${item.mapel}|${item.guru}|${item.foto_guru || ''}`).join('~')}`;
