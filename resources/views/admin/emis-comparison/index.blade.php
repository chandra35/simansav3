    ...
    <div class="simansa-emis-hero">
        ...
        <div class="simansa-emis-hero__meta">
            ...
            @can('sync-emis-comparison')
                <button type="button" id="btnSyncEmis" class="btn simansa-emis-sync-btn" @disabled(!$tokenStatus['usable'] || !$activeYear)>
...
@section('content')
    ...
    @if(!$activeYear)
        <div class="alert alert-danger simansa-period-alert"><i class="fas fa-calendar-times mr-2"></i><strong>Tahun pelajaran aktif belum ditetapkan.</strong> Snapshot baru dan sinkronisasi per siswa dinonaktifkan agar data tidak masuk ke periode yang salah.</div>
    @elseif(!$hasCurrentSnapshot)
        <div class="alert alert-warning simansa-period-alert"><i class="fas fa-layer-group mr-2"></i><strong>Belum ada snapshot untuk {{ $activeYear->nama }}.</strong> Jalankan sinkronisasi penuh sekali. Arsip periode lama tetap tersimpan dan tidak ikut dihapus.</div>
    @else
        <div class="simansa-period-strip mb-4"><div><i class="fas fa-calendar-check mr-2"></i><strong>Periode aktif: {{ $activeYear->nama }} · {{ $activeYear->semester_aktif }}</strong><span>Semua angka dan pembandingan di bawah hanya memakai snapshot periode ini.</span></div><span class="badge badge-success">Snapshot terpisah per tahun</span></div>
    @endif

    @if($archivePeriods->count() > 1)
        <details class="simansa-archive-panel mb-4">
            <summary><i class="fas fa-archive mr-2"></i>Arsip snapshot periode sebelumnya</summary>
            <div class="simansa-archive-grid">
                @foreach($archivePeriods as $period)
                    <div><strong>{{ $period->nama }}</strong><span>{{ $period->semester_aktif }} · {{ number_format($period->snapshot_count) }} data</span><small>{{ $period->is_active ? 'Periode aktif' : 'Arsip terlindungi' }}</small></div>
                @endforeach
            </div>
        </details>
    @endif
            ...
            <table class="table table-hover simansa-table mb-0">
                ...
                @forelse($items as $index => $item)
                    @php
                        ...
                        $snapshot = $listMode === 'simansa' ? $item->emisStudentSnapshots->first() : $item;
                                ...
                                @if($siswa)
                                    @can('sync-emis-comparison')
                                        <button type="button" class="btn btn-sm btn-outline-success btn-sync-emis-student"
                                                ...
                                                @disabled(!$tokenStatus['usable'] || !$hasCurrentSnapshot)
...
<style>
    ...
    .simansa-period-alert{border:0;border-left:4px solid currentColor;border-radius:12px;box-shadow:0 8px 22px rgba(15,23,42,.05)}.simansa-period-strip{display:flex;justify-content:space-between;align-items:center;gap:1rem;padding:.85rem 1rem;border:1px solid #bbf7d0;border-radius:12px;background:#f0fdf4;color:#166534}.simansa-period-strip span:not(.badge){display:block;margin-left:1.55rem;color:#64748b;font-size:.78rem}.simansa-archive-panel{border:1px solid #dbe4f0;border-radius:12px;background:#fff;overflow:hidden}.simansa-archive-panel summary{padding:.85rem 1rem;cursor:pointer;color:#334155;font-weight:700}.simansa-archive-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:.65rem;padding:0 1rem 1rem}.simansa-archive-grid>div{padding:.75rem;border-radius:10px;background:#f8fafc}.simansa-archive-grid strong,.simansa-archive-grid span,.simansa-archive-grid small{display:block}.simansa-archive-grid strong{color:#1e293b}.simansa-archive-grid span,.simansa-archive-grid small{color:#64748b;font-size:.75rem}
    ...
    @media(max-width:575.98px){.simansa-emis-hero__meta,.simansa-filter-panel{grid-template-columns:1fr}.simansa-emis-sync-btn,.simansa-filter-panel .btn{grid-column:auto}.simansa-section-head,.simansa-period-strip{flex-direction:column;align-items:flex-start}.simansa-progress-log-row{grid-template-columns:1fr}}
</style>
@stop
