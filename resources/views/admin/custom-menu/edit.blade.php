...
@section('title', 'Edit Custom Menu')
...
@section('content_header')
    <h1><i class="fas fa-edit"></i> Edit Custom Menu Siswa</h1>
@stop
...
$(document).ready(function() {
    ...
    tinymce.init({
        ...
        setup: function(editor) {
            editor.on('init', function() {
                editor.setContent(`{!! addslashes($customMenu->konten ?? '') !!}`);
            });
        }
    });

    // Load existing data
    $('#icon').val('{{ $customMenu->icon }}').trigger('change');
    $('#menu_group').val('{{ $customMenu->menu_group }}');
    $('#content_type').val('{{ $customMenu->content_type }}').trigger('change');
    $('#urutan').val('{{ $customMenu->urutan }}');
    $('#is_active').prop('checked', {{ $customMenu->is_active ? 'true' : 'false' }});

    // Load existing custom fields
    @if($customMenu->content_type === 'personal' && $customMenu->custom_fields)
        const existingFields = {!! json_encode($customMenu->getCustomFieldsArray()) !!};
        Object.entries(existingFields).forEach(([key, field]) => {
            fieldCounter++;
            const fieldHtml = `
                        ...
                        <div class="col-md-6">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" 
                                       ...
                                       value="1" ${field.show_in_table ? 'checked' : ''}>
    ...
    $('#form-edit-menu').submit(function(e) {
        ...
        $.ajax({
            url: '{{ route('admin.custom-menu.update', $customMenu->id) }}',
            type: 'POST',
            data: formData,
