...
<div class="simansa-polling-form">
    <form method="POST" action="{{ $editing ? route('admin.polling.update',$polling) : route('admin.polling.store') }}" id="pollingForm">
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card card-outline card-primary"><div class="card-header"><h3 class="card-title font-weight-bold"><i class="fas fa-heading text-primary mr-2"></i>Judul & Jadwal</h3></div><div class="card-body">
                ...
                <div class="card card-outline card-primary description-card"><div class="card-header"><h3 class="card-title font-weight-bold"><i class="fas fa-align-left text-primary mr-2"></i>Deskripsi & Petunjuk</h3></div><div class="card-body"><label for="pollDescription">Informasi untuk responden</label><textarea name="description" id="pollDescription" class="form-control" rows="7" placeholder="Jelaskan tujuan, petunjuk, atau informasi penting polling">{{ old('description',$polling->description??'') }}</textarea><small class="text-muted d-block mt-2"><i class="fas fa-info-circle mr-1"></i>Gunakan paragraf, daftar, tebal, atau miring agar deskripsi panjang mudah dibaca.</small></div></div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card card-outline card-primary"><div class="card-header"><h3 class="card-title font-weight-bold"><i class="fas fa-sliders-h text-primary mr-2"></i>Aturan</h3></div><div class="card-body">
                    <input type="hidden" name="allow_changes" value="0"><div class="custom-control custom-switch mb-3"><input type="checkbox" class="custom-control-input" id="allowChanges" name="allow_changes" value="1" @checked(old('allow_changes',$polling->allow_changes??true))><label class="custom-control-label" for="allowChanges">Izinkan permintaan buka kunci jawaban</label></div>
...
.simansa-polling-form .description-card .note-editor{border:1px solid #cbd5e1;border-radius:10px;overflow:hidden}.simansa-polling-form .description-card .note-toolbar{background:#f8fafc}.simansa-polling-form .description-card .note-editable{background:#fff;min-height:180px}.simansa-polling-form .rich-description p:last-child,.simansa-polling-form .rich-description ul:last-child,.simansa-polling-form .rich-description ol:last-child{margin-bottom:0}
...
$(function(){
    ...
    $('#pollDescription').summernote({height:180,placeholder:'Jelaskan tujuan, petunjuk, atau informasi penting polling',toolbar:[['style',['bold','italic','underline','clear']],['para',['ul','ol','paragraph']],['view',['codeview']]]});
    function safeRichText(html){const root=$('<div>').html(html||'');root.find('script,style,iframe,object,embed').remove();root.find('*').each(function(){const element=$(this),tag=this.tagName.toLowerCase();if(!['p','br','strong','b','em','i','u','ul','ol','li','blockquote'].includes(tag)){element.replaceWith(element.contents());return}Array.from(this.attributes).forEach(attribute=>this.removeAttribute(attribute.name))});return root.html()}
    function setDescription(value){$('#pollDescription').summernote('code',value&&value.includes('<')?value:`<p>${esc(value||'')}</p>`)}
    ...
    function renderPreview(){
        ...
        const description=safeRichText($('#pollDescription').summernote('code'))||'<p class="text-muted mb-0">Deskripsi polling akan tampil di sini.</p>';
        ...
        $('#pollingPreviewContent').html(`<div class="card bg-gradient-primary text-white preview-hero"><div class="card-body"><div class="row align-items-center"><div class="col-lg-8"><div class="small text-uppercase font-weight-bold mb-2"><i class="fas fa-clipboard-check mr-1"></i>${audience} · MODE PREVIEW</div><h2 class="h3 font-weight-bold mb-0">${title}</h2></div><div class="col-lg-4 mt-3 mt-lg-0"><div class="preview-deadline"><small>Batas Pengisian</small><strong class="d-block">${esc(deadline)} WIB</strong></div></div></div></div></div><div class="card card-outline card-primary"><div class="card-header"><h3 class="card-title font-weight-bold"><i class="fas fa-info-circle text-primary mr-2"></i>Informasi Polling</h3></div><div class="card-body rich-description">${description}</div></div>${questions}${consent}<div class="card mb-0"><div class="card-body d-flex justify-content-between align-items-center"><button type="button" class="btn btn-secondary" disabled><i class="fas fa-arrow-left mr-1"></i>Kembali</button><button type="button" class="btn btn-primary" disabled><i class="fas fa-paper-plane mr-1"></i>Kirim Jawaban</button></div></div>`);
    }
    $('#previewPolling').on('click',function(){renderPreview();$('#pollingPreviewModal').modal('show')});
    $('#tkaPreset').on('click',function(){Swal.fire({icon:'question',title:'Gunakan preset TKA?',text:'Identitas dan pertanyaan saat ini akan diganti dengan contoh pemilihan dua mapel TKA kelas XII.',showCancelButton:true,confirmButtonText:'Gunakan Preset',cancelButtonText:'Batal',confirmButtonColor:'#2563eb'}).then(r=>{if(!r.isConfirmed)return;$('#pollTitle').val('Pemilihan Mata Pelajaran Pilihan TKA 2026');setDescription('Pilih tepat dua mata pelajaran pilihan Tes Kemampuan Akademik yang tercatat di rapor serta sesuai minat dan rencana studi lanjut.');$('[name="audience"][value="siswa"]').prop('checked',true).trigger('change');$('#studentCustom').prop('checked',true).trigger('change');$('.grade-check,.class-check').prop('checked',false);$('#grade12').prop('checked',true);$('#allClasses').prop('checked',false);filterClasses();updateClassCount();$('#questionBuilder').empty();questionIndex=0;addQuestion({prompt:'Pilih tepat dua mata pelajaran pilihan TKA',type:'multiple',is_required:true,min_selections:2,max_selections:2,options_text:'Matematika Tingkat Lanjut\nBahasa Indonesia Tingkat Lanjut\nBahasa Inggris Tingkat Lanjut\nFisika\nKimia\nBiologi\nEkonomi\nSosiologi\nGeografi\nSejarah\nAntropologi\nPPKn/Pendidikan Pancasila\nBahasa Arab\nBahasa Jerman\nBahasa Prancis\nBahasa Jepang\nBahasa Korea\nBahasa Mandarin'});$('#requireConsent').prop('checked',true).trigger('change')})});
    function usePreset(config){
        Swal.fire({icon:'question',title:config.confirmTitle,text:'Identitas dan pertanyaan saat ini akan diganti.',showCancelButton:true,confirmButtonText:'Gunakan Preset',cancelButtonText:'Batal',confirmButtonColor:'#2563eb'}).then(r=>{
            ...
            $('#pollTitle').val(config.title);setDescription(config.description);
