...
@section('js')
<script>$(function(){$('#respondentSearch').on('input',function(){const q=this.value.toLowerCase();$('#respondentTable tbody tr').each(function(){$(this).toggle($(this).text().toLowerCase().includes(q))})});$('.confirm-close').on('submit',function(e){e.preventDefault();const f=this;Swal.fire({icon:'warning',title:'Tutup polling?',text:'Responden tidak dapat lagi mengirim jawaban.',showCancelButton:true,confirmButtonText:'Ya, tutup',cancelButtonText:'Batal',confirmButtonColor:'#64748b'}).then(r=>{if(r.isConfirmed)f.submit()})});$('.confirm-delete').on('submit',function(e){e.preventDefault();const f=this;Swal.fire({icon:'warning',title:'Hapus polling?',text:'Polling tanpa respons ini akan dihapus.',showCancelButton:true,confirmButtonText:'Hapus',cancelButtonText:'Batal',confirmButtonColor:'#dc2626'}).then(r=>{if(r.isConfirmed)f.submit()})})});</script>
@stop
