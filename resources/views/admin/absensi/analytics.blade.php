...
<div class="attendance-page pb-4">
    ...
    <form method="GET" class="filter-panel mt-3">
        ...
        <div class="form-group"><label>Kelas</label><select name="kelas_id" id="attendance-class" class="form-control"><option value="">Semua kelas</option>@foreach($classes as $class)<option value="{{ $class->id }}" data-level="{{ $class->tingkat }}" @selected($classId === $class->id)>{{ $class->nama_kelas }}{{ $class->asrama_suffix }}</option>@endforeach</select></div>
