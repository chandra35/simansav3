    ...
    @else
        ...
        @if($canBulkGenerate)
            <section class="attendance-bulk mb-4">
                <div><h2><i class="fas fa-layer-group mr-2"></i>Buat Draft Massal</h2><p>Buat sesi absensi harian berstatus draft dengan seluruh siswa awalnya Hadir. Sesi yang sudah ada tidak akan ditimpa.</p></div>
                <form method="POST" action="{{ route('admin.absensi-siswa.generate-draft') }}" id="bulkDraftForm">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fas fa-file-medical mr-1"></i>Buat Draft</button>
                </form>
            </section>
        @endif
...
<style>
.attendance-bulk{padding:1.25rem;border:1px solid #bfdbfe;border-radius:18px;background:#eff6ff}.attendance-bulk h2{margin:0;font-size:1.08rem;font-weight:800}.attendance-bulk p{margin:.25rem 0 1rem;color:#64748b}.attendance-bulk form{display:grid;grid-template-columns:180px 220px minmax(260px,1fr) auto;gap:.9rem;align-items:end}.attendance-bulk .form-group{margin:0}.attendance-bulk label{font-size:.75rem;text-transform:uppercase;color:#475569}.attendance-student-search{position:relative;width:260px}.attendance-student-search i{position:absolute;z-index:1;top:.52rem;left:.55rem;color:#94a3b8}.attendance-student-search input{padding-left:1.8rem}@media(max-width:767px){.attendance-bulk form{grid-template-columns:1fr}.attendance-student-search{width:100%}}
</style>
@stop
