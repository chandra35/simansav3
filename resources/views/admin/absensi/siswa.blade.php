    ...
    @else
        ...
        @if($canBulkGenerate)
            <section class="attendance-bulk mb-4">
                ...
                <form method="POST" action="{{ route('admin.absensi-siswa.generate-draft') }}" id="bulkDraftForm">
                    @csrf
                    <div class="form-group"><label>Tanggal</label><input type="date" name="tanggal" value="{{ $tanggal }}" max="{{ now()->format('Y-m-d') }}" class="form-control" required></div>
                    <div class="form-group"><label>Cakupan</label><select name="scope" id="bulkDraftScope" class="form-control"><option value="all">Semua kelas aktif</option><option value="selected">Kelas tertentu</option></select></div>
                    <div class="form-group bulk-draft-classes d-none"><label>Pilih kelas</label><select name="kelas_ids[]" class="form-control" multiple size="4">@foreach($kelasOptions as $kelas)<option value="{{ $kelas->id }}">Tingkat {{ $kelas->tingkat }} · {{ $kelas->nama_kelas }}</option>@endforeach</select></div>
                    <button type="submit" class="btn btn-primary attendance-bulk__submit"><i class="fas fa-file-medical mr-1"></i>Buat Draft</button>
                </form>
            </section>
        @endif
