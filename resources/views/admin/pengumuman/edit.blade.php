...
@section('content_header')
    <h1><i class="fas fa-edit mr-2"></i>Edit Pengumuman</h1>
@stop
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="isi" id="isi" class="form-control @error('isi') is-invalid @enderror" 
                                rows="10" required>{{ old('isi', $pengumuman->isi) }}</textarea>
                            @error('isi')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="prioritas" id="prioritas" class="form-control @error('prioritas') is-invalid @enderror" required>
                                @foreach($prioritas as $key => $label)
                                    <option value="{{ $key }}" {{ old('prioritas', $pengumuman->prioritas) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('prioritas')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="form-group">
                            ...
                            <select name="target" id="target" class="form-control @error('target') is-invalid @enderror" required>
                                @foreach($target as $key => $label)
                                    <option value="{{ $key }}" {{ old('target', $pengumuman->target) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('target')
                                <span class="invalid-feedback">{{ $message }}</span>
