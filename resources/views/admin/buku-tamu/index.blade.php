...
<style>
    ...
    .detail-item--primary { padding: .9rem 1rem; background: #fff; border-color: #cbd5e1; box-shadow: 0 5px 14px rgba(15,23,42,.04); }
    .detail-item--primary strong { font-size: 1rem; line-height: 1.5; }
    .detail-item--meta { grid-column: 1 / -1; padding: .75rem .85rem; border-style: dashed; background: #f8fafc; }
    .detail-item--meta small { display: block; margin-bottom: .5rem; color: #94a3b8; font-size: .62rem; }
    .detail-meta-list { display: flex; flex-wrap: wrap; gap: .45rem .9rem; }
    .detail-meta-entry { color: #64748b; font-size: .76rem; }
    .detail-meta-entry strong { color: #475569; font-weight: 700; }
    .detail-meta-user-agent { display: block; width: 100%; margin-top: .25rem; overflow-wrap: anywhere; }
    .guest-phone-actions { display: flex; flex-wrap: wrap; align-items: center; gap: .45rem; }
    .guest-phone-actions .btn { font-size: .75rem; }
...
$(function () {
    ...
    $('.js-guest-detail').on('click', function () {
        ...
        $.get(button.data('url')).done(function (data) { Object.keys(data).forEach(function (key) { $('[data-field="' + key + '"]').text(data[key] || '-'); }); $('#guestDetailTime').text(data.waktu || '-'); const phone = data.nomor_hp || ''; $('#guestWhatsapp').attr('href', phone ? 'https://wa.me/' + phone.replace(/[^0-9]/g, '').replace(/^0/, '62') : '#').toggleClass('disabled', !phone); $('#guestCopyPhone').data('phone', phone).prop('disabled', !phone); $('#guestDetailLoading').addClass('d-none'); $('#guestDetailContent').removeClass('d-none'); }).fail(function () { $('#guestDetailLoading').addClass('d-none'); $('#guestDetailError').removeClass('d-none'); });
    });
    $('#guestCopyPhone').on('click', function () { const phone = $(this).data('phone'); if (!phone) return; navigator.clipboard.writeText(phone).then(function () { Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Nomor HP berhasil disalin', showConfirmButton: false, timer: 1800 }); }); });
