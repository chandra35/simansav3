...
<div class="container-fluid px-0">
    ...
    <div class="card card-outline card-primary">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive"><table class="table table-hover table-striped"><thead><tr><th>Tanggal</th><th>Nama</th><th>Instansi/Alamat</th><th>Nomor HP</th><th>Keperluan</th>@can('delete-buku-tamu')<th>Aksi</th>@endcan</tr></thead><tbody>@forelse($items as $item)<tr><td>{{ $item->tanggal_kunjungan?->format('d-m-Y') }}</td><td>{{ $item->nama }}</td><td>{{ $item->alamat_instansi }}</td><td>{{ $item->nomor_hp }}</td><td>{{ $item->keperluan }}</td>@can('delete-buku-tamu')<td><form method="POST" action="{{ route('admin.buku-tamu.destroy', $item) }}" onsubmit="return confirm('Arsipkan data kunjungan ini?')">@csrf @method('DELETE')<button class="btn btn-outline-danger btn-sm" title="Arsipkan"><i class="fas fa-archive"></i></button></form></td>@endcan</tr>@empty<tr><td colspan="6" class="text-center text-muted">Belum ada data kunjungan.</td></tr>@endforelse</tbody></table></div>
            {{ $items->links() }}
        </div>
    </div>
    <div class="card card-outline card-secondary"><div class="card-header"><h3 class="card-title"><i class="fas fa-qrcode mr-1"></i> QR Code PTSP</h3></div><div class="card-body text-center"><div class="mb-2">{!! $qrSvg !!}</div><code>{{ $publicUrl }}</code><p class="text-muted mt-2 mb-0">Cetak QR ini dan tempatkan di meja PTSP.</p></div></div>
</div>
@stop
