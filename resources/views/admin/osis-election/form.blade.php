...
<div class="col-xl-4">
    ...
    <section class="form-panel mb-4"><div class="form-panel-title"><i class="fas fa-users"></i><div><h2>Hak Pilih</h2><p>Daftar pemilih dibekukan saat publikasi.</p></div></div>
        <label>Kelompok yang Boleh Memilih</label><div class="level-options">@foreach([10=>'Kelas X',11=>'Kelas XI',12=>'Kelas XII'] as $level=>$label)<label><input type="checkbox" name="eligible_levels[]" value="{{ $level }}" @checked(in_array($level,old('eligible_levels',$election->eligible_levels ?: [10,11,12])))><span>{{ $label }}</span></label>@endforeach</div>
        <input type="hidden" name="include_gtk" value="0"><label class="gtk-voter-option"><input type="checkbox" name="include_gtk" value="1" @checked((bool) old('include_gtk',$election->include_gtk))><span><i class="fas fa-chalkboard-teacher"></i><b>Sertakan GTK</b><small>Guru dan tenaga kependidikan aktif ikut masuk daftar pemilih tetap.</small></span></label>
