<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>ID Card GTK - {{ $setting->nama_sekolah ?? 'SIMANSA' }}</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 5mm 6mm;
        }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: 'Arial', 'Helvetica Neue', sans-serif;
            font-size: 7pt;
            line-height: 1.3;
            color: #333333;
        }
        .page-break { page-break-after: always; }
        .page-break:last-child { page-break-after: avoid; }

        /* === GRID: front+back side by side === */
        .card-grid {
            border-collapse: separate;
            border-spacing: 1.5mm;
        }
        .card-grid td {
            vertical-align: top;
            padding: 0;
        }
        .pair-gap {
            width: 10mm;
        }

        /* === CARD SHELL (62mm × 98mm) === */
        .id-card {
            width: 62mm;
            height: 98mm;
            overflow: hidden;
            background-size: 62mm 98mm;
            background-repeat: no-repeat;
            background-position: center;
        }

        /* ====== FRONT (ASN-style: white top, gradient bottom) ====== */
        .f-header {
            text-align: center;
            padding: 3mm 3mm 1.5mm 3mm;
        }
        .f-header-tbl {
            width: 100%;
            border-collapse: collapse;
        }
        .f-header-tbl td {
            vertical-align: middle;
            padding: 0;
        }
        .f-logo {
            width: 11mm;
            text-align: center;
        }
        .f-logo img {
            height: 10mm;
            width: auto;
        }
        .f-htext {
            text-align: center;
            padding: 0 1mm;
        }
        .f-ministry {
            font-size: 6.5pt;
            color: #888888;
            letter-spacing: 0.3px;
            text-transform: uppercase;
        }
        .f-school {
            font-size: 8.5pt;
            font-weight: bold;
            color: #1a3a4a;
            text-transform: uppercase;
            letter-spacing: 0.3px;
        }
        .f-addr {
            font-size: 5pt;
            color: #aaaaaa;
        }
        .f-separator {
            border: none;
            margin: 0;
            height: 0;
        }

        /* Large photo area — borderless, fills white zone */
        .f-foto-area {
            text-align: center;
            padding: 0.5mm 0 0 0;
        }
        .f-foto-frame {
            width: 44mm;
            height: 50mm;
            overflow: hidden;
            text-align: center;
            margin: 0 auto;
            background: transparent;
        }
        .f-foto-frame img {
            width: 44mm;
            height: 50mm;
        }
        .f-foto-placeholder {
            padding-top: 21mm;
            font-size: 6pt;
            color: #cccccc;
            height: 50mm;
        }

        /* Bottom section on gradient */
        .f-bottom {
            padding: 3mm 4.5mm 0 4.5mm;
        }
        .f-bottom-tbl {
            width: 100%;
            border-collapse: collapse;
        }
        .f-bottom-tbl td {
            vertical-align: middle;
            padding: 0;
        }
        .f-info {
            text-align: left;
        }
        .f-name {
            font-size: 9pt;
            font-weight: bold;
            color: #ffffff;
            letter-spacing: 0.4px;
            padding-bottom: 1mm;
            line-height: 1.3;
        }
        .f-id-number {
            font-size: 6pt;
            color: rgba(255,255,255,0.7);
            letter-spacing: 0.8px;
        }
        .f-qr-cell {
            width: 17mm;
            text-align: right;
        }
        .f-qr-box {
            width: 15mm;
            height: 15mm;
        }
        .f-qr-box img {
            width: 15mm;
            height: 15mm;
        }

        /* ====== BACK (full gradient) ====== */
        .b-header {
            text-align: center;
            padding: 3.5mm 3.5mm 2mm 3.5mm;
        }
        .b-header-title {
            font-size: 6pt;
            font-weight: bold;
            color: #ffffff;
            letter-spacing: 0.5px;
            text-transform: uppercase;
        }
        .b-header-sub {
            font-size: 4pt;
            color: rgba(255,255,255,0.7);
            padding-top: 0.3mm;
        }
        .b-id-number {
            text-align: right;
            padding: 0 3.5mm;
            font-size: 5pt;
            font-weight: bold;
            color: rgba(255,255,255,0.8);
            letter-spacing: 0.3px;
        }
        .b-body {
            padding: 1.5mm 3.5mm 1mm 3.5mm;
        }
        .b-section-title {
            font-size: 5.5pt;
            font-weight: bold;
            color: rgba(255,255,255,0.9);
            text-transform: uppercase;
            letter-spacing: 0.3px;
            border-bottom: 0.4px solid rgba(255,255,255,0.3);
            padding-bottom: 0.3mm;
            margin-bottom: 0.5mm;
        }
        .b-detail-tbl {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 1.5mm;
        }
        .b-detail-tbl td {
            font-size: 5.5pt;
            padding: 0.4mm 0;
            vertical-align: top;
            color: rgba(255,255,255,0.9);
        }
        .b-detail-tbl .lbl {
            width: 14mm;
            color: rgba(255,255,255,0.6);
        }
        .b-detail-tbl .sep {
            width: 2mm;
            text-align: center;
            color: rgba(255,255,255,0.4);
        }
        .b-rules-title {
            font-size: 5.5pt;
            font-weight: bold;
            color: rgba(255,255,255,0.9);
            text-transform: uppercase;
            letter-spacing: 0.3px;
            border-bottom: 0.4px solid rgba(255,255,255,0.3);
            padding-bottom: 0.3mm;
            margin-bottom: 0.5mm;
            margin-top: 1mm;
        }
        .b-rules ol {
            margin: 0;
            padding-left: 3.5mm;
            font-size: 5pt;
            color: rgba(255,255,255,0.8);
            line-height: 1.5;
        }
        .b-sig-tbl {
            width: 100%;
            border-collapse: collapse;
            margin-top: 2mm;
        }
        .b-sig-tbl td {
            text-align: center;
            vertical-align: bottom;
            font-size: 5pt;
            width: 50%;
            color: rgba(255,255,255,0.8);
            padding: 0 2mm;
        }
        .b-sig-line {
            border-bottom: 0.4px solid rgba(255,255,255,0.5);
            margin-top: 6mm;
            margin-bottom: 0.3mm;
        }
        .b-sig-name {
            font-weight: bold;
            font-size: 5pt;
            color: #ffffff;
        }
        .b-footer {
            text-align: center;
            padding: 1.5mm 2mm 1mm 2mm;
        }
        .b-footer-text {
            font-size: 5pt;
            font-weight: bold;
            color: rgba(255,255,255,0.9);
            letter-spacing: 0.3px;
        }
        .b-footer-sub {
            font-size: 3pt;
            color: rgba(255,255,255,0.5);
        }
    </style>
</head>
<body>
@php
    // 2 pairs per row × 2 rows = 4 persons per A4 landscape page
    $chunks = $gtkList->chunk(4);
@endphp

@foreach($chunks as $chunkIndex => $chunk)
<div class="{{ !$loop->last ? 'page-break' : '' }}">
    <table class="card-grid">
    @foreach($chunk->chunk(2) as $row)
        <tr>
        @foreach($row as $rowIdx => $gtk)
            @if($rowIdx > 0)
            <td class="pair-gap"></td>
            @endif
            {{-- FRONT (ASN-style: white + gradient bottom) --}}
            <td>
                <div class="id-card" style="background-image: url('{{ $bgFrontBase64 }}');">
                    <div class="f-header">
                        <table class="f-header-tbl">
                            <tr>
                                @if($logoKemenagBase64)
                                <td class="f-logo">
                                    <img src="{{ $logoKemenagBase64 }}" alt="">
                                </td>
                                @endif
                                <td class="f-htext">
                                    <div class="f-ministry">Kementerian Agama RI</div>
                                    <div class="f-school">{{ $setting->nama_sekolah ?? 'Nama Sekolah' }}</div>
                                    <div class="f-addr">{{ Illuminate\Support\Str::limit($setting->alamat ?? '', 55) }}</div>
                                </td>
                                @if($logoSekolahBase64)
                                <td class="f-logo">
                                    <img src="{{ $logoSekolahBase64 }}" alt="">
                                </td>
                                @endif
                            </tr>
                        </table>
                    </div>
                    <hr class="f-separator">

                    {{-- Large photo --}}
                    <div class="f-foto-area">
                        <div class="f-foto-frame">
                            @if($gtk->foto_base64)
                                <img src="{{ $gtk->foto_base64 }}" alt="">
                            @else
                                <div class="f-foto-placeholder">Foto 3&times;4</div>
                            @endif
                        </div>
                    </div>

                    {{-- Bottom: Name + NIP + QR --}}
                    <div class="f-bottom">
                        <table class="f-bottom-tbl">
                            <tr>
                                <td class="f-info">
                                    <div class="f-name">{{ strtoupper($gtk->nama_lengkap) }}</div>
                                    <div class="f-id-number">{{ $gtk->nip ?? ($gtk->nuptk ?? '-') }}</div>
                                </td>
                                @if($gtk->qr_base64)
                                <td class="f-qr-cell">
                                    <div class="f-qr-box">
                                        <img src="{{ $gtk->qr_base64 }}" alt="">
                                    </div>
                                </td>
                                @endif
                            </tr>
                        </table>
                    </div>
                </div>
            </td>
            {{-- BACK (full gradient) --}}
            <td>
                <div class="id-card" style="background-image: url('{{ $bgBackBase64 }}');">
                    <div class="b-header">
                        <div class="b-header-title">{{ $setting->nama_sekolah ?? 'Nama Sekolah' }}</div>
                        <div class="b-header-sub">Kementerian Agama Republik Indonesia</div>
                    </div>
                    <div class="b-id-number">{{ $gtk->nip ?? ($gtk->nuptk ?? '') }}</div>
                    <div class="b-body">
                        <div class="b-section-title">Data Pribadi</div>
                        <table class="b-detail-tbl">
                            <tr>
                                <td class="lbl">NIK</td>
                                <td class="sep">:</td>
                                <td>{{ $gtk->nik ?? '-' }}</td>
                            </tr>
                            <tr>
                                <td class="lbl">NUPTK</td>
                                <td class="sep">:</td>
                                <td>{{ $gtk->nuptk ?? '-' }}</td>
                            </tr>
                            <tr>
                                <td class="lbl">NIP</td>
                                <td class="sep">:</td>
                                <td>{{ $gtk->nip ?? '-' }}</td>
                            </tr>
                            <tr>
                                <td class="lbl">TTL</td>
                                <td class="sep">:</td>
                                <td>{{ $gtk->tempat_lahir ?? '-' }}, {{ $gtk->tanggal_lahir ? $gtk->tanggal_lahir->format('d-m-Y') : '-' }}</td>
                            </tr>
                            <tr>
                                <td class="lbl">Alamat</td>
                                <td class="sep">:</td>
                                <td>{{ \Illuminate\Support\Str::limit($gtk->alamat ?? '-', 35) }}</td>
                            </tr>
                        </table>
                        <div class="b-rules-title">Ketentuan</div>
                        <div class="b-rules">
                            <ol>
                                <li>Kartu ini wajib dibawa saat di lingkungan sekolah</li>
                                <li>Tidak dapat dipindahtangankan</li>
                                <li>Jika hilang segera melapor ke sekolah</li>
                                <li>Berlaku selama menjadi GTK aktif</li>
                            </ol>
                        </div>
                        <table class="b-sig-tbl">
                            <tr>
                                <td>
                                    Pemegang Kartu
                                    <div class="b-sig-line"></div>
                                </td>
                                <td>
                                    Kepala Madrasah
                                    <div class="b-sig-line"></div>
                                    <div class="b-sig-name">{{ $setting->nama_kepala_sekolah ?? '' }}</div>
                                </td>
                            </tr>
                        </table>
                    </div>
                    <div class="b-footer">
                        @if($setting->website)
                            <div class="b-footer-text">{{ $setting->website }}</div>
                        @endif
                        <div class="b-footer-sub">SIMANSA v3 &bull; {{ $setting->nama_sekolah ?? '' }}</div>
                    </div>
                </div>
            </td>
        @endforeach
        {{-- Fill empty cells if odd number --}}
        @if($row->count() < 2)
            <td class="pair-gap"></td>
            <td></td>
            <td></td>
        @endif
        </tr>
    @endforeach
    </table>
</div>
@endforeach
</body>
</html>
