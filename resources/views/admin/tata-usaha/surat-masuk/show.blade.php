...
<div class="modal fade" id="suratAsliPreviewModal" tabindex="-1" role="dialog" aria-labelledby="suratAsliPreviewModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content disposisi-preview-modal">
            <div class="modal-header">
                <div><h5 class="modal-title mb-1" id="suratAsliPreviewModalLabel"><i class="fas fa-paperclip text-secondary mr-2"></i>Preview Surat Masuk Asli</h5><small class="text-muted">{{ $item->surat_masuk_nama ?: 'Berkas surat masuk' }}</small></div>
                <button type="button" class="close" data-dismiss="modal" aria-label="Tutup"><span aria-hidden="true">&times;</span></button>
            </div>
            <div class="modal-body p-0"><div class="disposisi-preview-frame"><div class="disposisi-preview-loading"><i class="fas fa-spinner fa-spin mr-2"></i>Menyiapkan preview berkas...</div><iframe id="suratAsliPreviewFrame" title="Preview Surat Masuk Asli" loading="lazy"></iframe></div></div>
            <div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-dismiss="modal"><i class="fas fa-times mr-1"></i>Tutup</button><a href="{{ route('admin.tata-usaha.surat-masuk.download', [$item, 'surat-masuk']) }}" data-no-overlay class="btn btn-primary"><i class="fas fa-download mr-1"></i>Download Berkas</a></div>
        </div>
    </div>
</div>
@stop
...
$(function(){
    ...
    const suratModal=$('#suratAsliPreviewModal'),suratFrame=$('#suratAsliPreviewFrame'),suratPreviewUrl=@json(route('admin.tata-usaha.surat-masuk.download',[$item,'surat-masuk']).'?preview=1');
    ...
    suratModal.on('show.bs.modal',function(){suratFrame.attr('src',suratPreviewUrl);});
    suratModal.on('hidden.bs.modal',function(){suratFrame.attr('src','');window.hideAppGlobalOverlay&&window.hideAppGlobalOverlay();});
    suratModal.on('shown.bs.modal',function(){window.hideAppGlobalOverlay&&window.hideAppGlobalOverlay();});
