...
<div class="gtk-my-schedule">
    ...
    <div class="card card-outline card-primary"><div class="card-header"><h3 class="card-title"><i class="fas fa-list mr-1"></i> Jadwal Mingguan</h3></div><div class="card-body">
        @if($schedulesByDay->isNotEmpty())<div class="row">@foreach($dayLabels as $day => $label)<div class="col-md-6 col-xl-4 mb-3"><section class="gtk-my-schedule__day"><header>{{ $label }} <span>{{ ($schedulesByDay[$day] ?? collect())->count() }} slot</span></header><div>@forelse($schedulesByDay[$day] ?? collect() as $schedule)<article><time>{{ $schedule->jam }}</time><div><strong>{{ $schedule->mataPelajaran?->nama_mapel ?? 'Mata pelajaran' }}</strong><span>{{ $schedule->location_label }}</span></div></article>@empty<span class="gtk-my-schedule__empty">Tidak ada jadwal</span>@endforelse</div></section></div>@endforeach</div>@else<div class="text-center text-muted py-5"><i class="far fa-calendar-times fa-3x mb-3 d-block"></i>Belum ada jadwal mengajar aktif pada tahun pelajaran ini.</div>@endif
    </div></div>
</div>
@stop
