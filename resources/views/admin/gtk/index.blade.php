...
<div class="card simansa-management-card">
    <div class="card-header">
        ...
        <div class="card-tools ml-0">
            @can('edit-gtk')
                <button type="button" class="btn btn-light text-dark btn-sm mr-1" id="btnBulkSyncKemenag">
                    <i class="fas fa-sync-alt"></i> Sinkron Semua GTK Ber-NIP
                    <span class="badge badge-secondary ml-1">{{ $stats['gtk_with_nip'] }}</span>
                </button>
            @endcan
            @can('create-gtk')
                <a href="{{ route('admin.gtk.import') }}" class="btn btn-light text-dark btn-sm mr-1">
                    <i class="fas fa-file-excel"></i> Import GTK
                </a>
                <button type="button" class="btn btn-light text-dark btn-sm" data-toggle="modal" data-target="#addGtkModal">
