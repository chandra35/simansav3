...
@section('content_header')
    <h1><i class="fas fa-edit mr-2"></i>Edit Catatan Konseling</h1>
@stop
...
@section('content')
    <div class="card">
        <form action="{{ route('admin.catatan-konseling.update', $catatanKonseling->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="siswa_id" id="siswa_id" class="form-control select2 @error('siswa_id') is-invalid @enderror" required>
                                ...
                                @foreach($siswa as $s)
                                    <option value="{{ $s->id }}" {{ old('siswa_id', $catatanKonseling->siswa_id) == $s->id ? 'selected' : '' }}>
                                        {{ $s->nis }} - {{ $s->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="tahun_pelajaran_id" id="tahun_pelajaran_id" class="form-control @error('tahun_pelajaran_id') is-invalid @enderror" required>
                                @foreach($tahunPelajaran as $tp)
                                    <option value="{{ $tp->id }}" {{ old('tahun_pelajaran_id', $catatanKonseling->tahun_pelajaran_id) == $tp->id ? 'selected' : '' }}>
                                        {{ $tp->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="konselor_id" id="konselor_id" class="form-control select2 @error('konselor_id') is-invalid @enderror" required>
                                ...
                                @foreach($konselor as $k)
                                    <option value="{{ $k->id }}" {{ old('konselor_id', $catatanKonseling->konselor_id) == $k->id ? 'selected' : '' }}>
                                        {{ $k->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="tanggal_konseling" id="tanggal_konseling" 
                                ...
                                value="{{ old('tanggal_konseling', $catatanKonseling->tanggal_konseling?->format('Y-m-d')) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select name="jenis_konseling" id="jenis_konseling" class="form-control @error('jenis_konseling') is-invalid @enderror" required>
                                @foreach($jenis as $key => $label)
                                    <option value="{{ $key }}" {{ old('jenis_konseling', $catatanKonseling->jenis_konseling) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="kategori_masalah" id="kategori_masalah" class="form-control @error('kategori_masalah') is-invalid @enderror" required>
                                @foreach($kategori as $key => $label)
                                    <option value="{{ $key }}" {{ old('kategori_masalah', $catatanKonseling->kategori_masalah) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea name="deskripsi_masalah" id="deskripsi_masalah" 
                                ...
                                rows="4" required>{{ old('deskripsi_masalah', $catatanKonseling->deskripsi_masalah) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="hasil_konseling" id="hasil_konseling" 
                                ...
                                rows="4">{{ old('hasil_konseling', $catatanKonseling->hasil_konseling) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="tindak_lanjut" id="tindak_lanjut" 
                                ...
                                rows="3">{{ old('tindak_lanjut', $catatanKonseling->tindak_lanjut) }}</textarea>
                            @error('tindak_lanjut')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control @error('status') is-invalid @enderror" required>
                                @foreach($status as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $catatanKonseling->status) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('status')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="jadwal_tindak_lanjut" id="jadwal_tindak_lanjut" 
                                ...
                                value="{{ old('jadwal_tindak_lanjut', $catatanKonseling->jadwal_tindak_lanjut?->format('Y-m-d')) }}">
                        ...
                        <div class="form-group">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" name="is_rahasia" id="is_rahasia" class="custom-control-input" value="1" 
                                    {{ old('is_rahasia', $catatanKonseling->is_rahasia) ? 'checked' : '' }}>
