...
<div class="asrama-panel"><div class="table-responsive"><table class="table asrama-table"><thead><tr><th>GTK</th><th>Jabatan</th><th>Kewenangan</th><th>Beban Aktif</th><th>Status</th><th></th></tr></thead><tbody>
@forelse($records as $item)
<tr><td><strong>{{ $item->gtk->nama_lengkap }}</strong><br><small>{{ $item->gtk->nip ?: $item->gtk->nuptk }}</small></td><td>{{ $item->jabatan }}</td><td>
...
@empty<tr><td colspan="6" class="asrama-empty"><i class="fas fa-chalkboard-teacher"></i>Belum ada GTK yang ditugaskan di Asrama.</td></tr>@endforelse
...
<div class="modal-header"><div><h5 class="modal-title">{{ $item->gtk->nama_lengkap }}</h5><small class="text-white-50">Atur kewenangan tugas GTK.</small></div><button type="button" class="close" data-dismiss="modal">&times;</button></div><div class="modal-body"><div class="row">
<div class="col-12"><label>Jabatan</label><input required name="jabatan" class="form-control" value="{{ $item->jabatan }}"></div>
...
<div class="col-12 mt-3"><div class="custom-control custom-switch"><input id="asatidzActive{{ $item->id }}" type="checkbox" name="is_active" value="1" class="custom-control-input" @checked($item->is_active)><label for="asatidzActive{{ $item->id }}" class="custom-control-label">Penugasan aktif</label></div></div>
