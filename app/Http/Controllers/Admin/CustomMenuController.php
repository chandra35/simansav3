...
class CustomMenuController extends Controller
{
    ...
    /**
     * Get siswa list for manual selection (AJAX)
     */
    public function getSiswaList(Request $request, CustomMenu $customMenu)
    {

        $search = $request->get('search', '');
        $assignedSiswaIds = $customMenu->menuSiswa->pluck('siswa_id')->toArray();

        $query = Siswa::with('kelasAktif')
        ...
        $siswa = $query->orderBy('nama_lengkap')
            ->limit(100)
            ->get()
            ->map(function($s) use ($assignedSiswaIds) {
                $kelasAktif = $s->kelasAktif->first();
                return [
                    'id' => $s->id,
                    'nisn' => $s->nisn,
                    'nama_lengkap' => $s->nama_lengkap,
                    'kelas' => $kelasAktif ? $kelasAktif->nama_lengkap : '-',
                    'is_assigned' => in_array($s->id, $assignedSiswaIds),
                ];
            });
     ...
     */
    public function getSiswaByKelas(Request $request, CustomMenu $customMenu)
    {

        $kelasId = $request->get('kelas_id');
        $assignedSiswaIds = $customMenu->menuSiswa->pluck('siswa_id')->toArray();

        $siswa = Siswa::whereHas('kelasAktif', function($q) use ($kelasId) {
